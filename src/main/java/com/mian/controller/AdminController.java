...
public class AdminController extends BaseController {
    ...
    public String updateAdmin(Admin update, String birthdayStr, String flag) {
        ...
        if ("other".equals(flag)) {
            ...
            boolean InfoEquals = loginAdmin.getInfo().equals(update.getInfo());
            update.setInfo(InfoEquals ? null : update.getInfo());
            if (InfoEquals) {
                result.setCode(501);
                logger.warn("【失败】没有修改任何信息");
                return JSONObject.toJSONString(result);
     ...
     * 清理并重建ElasticSearch缓存
     ...
     @GetMapping("/resetEs") public String resetEs(){
     ...
     * 清空Redis数据库
     ...
     @GetMapping("/cleanRedis") public String cleanRedis(){
